...
//		int movesToNextRowMe = movesToNextRow("M",vertices, edges);
		return (float) distAI_1 - distAI_2; // + (movesToNextRowMe - movesToNextRowAI)/4;
	}
...
//		avMoves = CommonMethods.shuffle(avMoves);
		...
		if(player == AI_1.colour){
			...
			for(int l=0; l < avMoves.size(); l++){
				...
				Move result=Minimax_AI_2.main(oppositePlayer,verticesCopy,edgesCopy,vertical_tiles_copy,horizontal_tiles_copy,depth+1,alpha,beta,walls_ai_2_copy,walls_ai_1_copy);
