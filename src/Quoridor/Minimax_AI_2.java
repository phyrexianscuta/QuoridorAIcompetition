...
public class Minimax_AI_2 {
	...
	public static float evaluatingfunction(ArrayList<Object> vertices, ArrayList<int[]> edges ) {
		...
		int movesToNextRow_AI_1 = CommonMethods.movesToNextRow(AI_1.colour,vertices, edges);
		int movesToNextRow_AI_2 = CommonMethods.movesToNextRow(AI_2.colour,vertices, edges);
		return (float) distAI_1 - distAI_2 + (movesToNextRow_AI_1 - movesToNextRow_AI_2)/4;
	}
	...
	public static Move main(int turns, char player,ArrayList<Object> vertices,ArrayList<int[]> edges,int[] vertical_tiles,int[] horizontal_tiles,int depth,float alpha,float beta,int walls_ai_2,int walls_ai_1) throws FileNotFoundException {
		...
		else if( AI_2.AI_2_Wins(vertices) ){
			Move x = new Move();
			x.score = 100-depth;
			return x;
		}
		...
		if(player == AI_1.colour){
			...
			for(int l=0; l < avMoves.size(); l++){
				...
				if(avMoves.get(l).kind== "P"){
					...
					walls_ai_1_copy -= 1;
					if(move.index+1<64 && move.index/8 == (move.index+1)/8 ){
					...
					if( CommonMethods.lookFor(new int[] {vertex_top_left+9,vertex_top_left+10}, edgesCopy) <0 ) {
						System.out.println("error minimax ai_2");
					}
					edgesCopy.remove( CommonMethods.lookFor(new int[] {vertex_top_left+9,vertex_top_left+10}, edgesCopy) );

										
				}
				Move result=Minimax_AI_2.main(turns,oppositePlayer,verticesCopy,edgesCopy,vertical_tiles_copy,horizontal_tiles_copy,depth+1,alpha,beta,walls_ai_2_copy,walls_ai_1_copy);
	...
	//		if(depth==0) {
	//			int tuto = turns +1;
	//			ff= new PrintWriter("Scores "+tuto+" AI_2.txt");
	//		}
			bestMove.score =-999f;
			for(int l=0;l< avMoves.size();l++){
				Move move= new Move();
